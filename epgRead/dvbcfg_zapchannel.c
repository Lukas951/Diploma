	...
	while ((line_len = getline(&line_buf, &line_size, file)) > 0) {
		...
		*line_tmp = '\0';
		...
		case DVBFE_TYPE_ATSC:
			...
			/* modulation */
			tmp.fe_params.u.atsc.modulation = static_cast<dvbfe_atsc_mod>(dvbcfg_parse_setting(&line_pos, ":", dvbcfg_atsc_modulation_list));
			if (!line_pos)
				continue;
		...
		case DVBFE_TYPE_DVBC:
			/* inversion */
			tmp.fe_params.inversion = static_cast<dvbfe_spectral_inversion>(dvbcfg_parse_setting(&line_pos, ":", dvbcfg_inversion_list));
			if (!line_pos)
				continue;
			...
			/* fec */
			tmp.fe_params.u.dvbc.fec_inner = static_cast<dvbfe_code_rate>(dvbcfg_parse_setting(&line_pos, ":", dvbcfg_fec_list));
			if (!line_pos)
				continue;

			/* modulation */
			tmp.fe_params.u.dvbc.modulation = static_cast<dvbfe_dvbc_mod>(dvbcfg_parse_setting(&line_pos, ":", dvbcfg_dvbc_modulation_list));
			if (!line_pos)
				continue;
		...
		case DVBFE_TYPE_DVBS:
			...
			/* symbol rate */
			tmp.fe_params.u.dvbs.symbol_rate = dvbcfg_parse_int(&line_pos, ":") * 1000;
			if (!line_pos)
				continue;
		...
		case DVBFE_TYPE_DVBT:
			...
			/* bandwidth */
			tmp.fe_params.u.dvbt.bandwidth = static_cast<dvbfe_dvbt_bandwidth>(dvbcfg_parse_setting(&line_pos, ":", dvbcfg_bandwidth_list));
			if (!line_pos)
				continue;

			/* fec hp */
			tmp.fe_params.u.dvbt.code_rate_HP = static_cast<dvbfe_code_rate>(dvbcfg_parse_setting(&line_pos, ":", dvbcfg_fec_list));
			if (!line_pos)
				continue;

			/* fec lp */
			tmp.fe_params.u.dvbt.code_rate_LP = static_cast<dvbfe_code_rate>(dvbcfg_parse_setting(&line_pos, ":", dvbcfg_fec_list));
			if (!line_pos)
				continue;

			/* constellation */
			tmp.fe_params.u.dvbt.constellation = static_cast<dvbfe_dvbt_const>(dvbcfg_parse_setting(&line_pos, ":", dvbcfg_constellation_list));
			if (!line_pos)
				continue;

			/* transmission mode */
			tmp.fe_params.u.dvbt.transmission_mode = static_cast<dvbfe_dvbt_transmit_mode>(dvbcfg_parse_setting(&line_pos, ":", dvbcfg_transmission_mode_list));
			if (!line_pos)
				continue;

			/* guard interval */
			tmp.fe_params.u.dvbt.guard_interval = static_cast<dvbfe_dvbt_guard_interval>(dvbcfg_parse_setting(&line_pos, ":", dvbcfg_guard_interval_list));
			if (!line_pos)
				continue;

			/* hierarchy */
			tmp.fe_params.u.dvbt.hierarchy_information = static_cast<dvbfe_dvbt_hierarchy>(dvbcfg_parse_setting(&line_pos, ":", dvbcfg_hierarchy_list));
			if (!line_pos)
				continue;
